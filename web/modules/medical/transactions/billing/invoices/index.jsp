    ...
    <body>
        ...
        <script type="text/javascript">
            ...
            var invoice = {
                ...
                */
                editInvoice: function(invNo){
                    module.execute('editInvoice', 'regNo='+ $F('regNo')+ "&invNo="+ invNo, 'divInvoices');
                },
                addInvItem: function(){
                    module.execute('addInvItem', 'regNo='+ $F('regNo')+ "&invNo="+ $F('invNo'), 'divInvDtls');
                },
                editInvItem: function(id){
                    module.execute('addInvItem', 'regNo='+ $F('regNo')+ "&invNo="+ $F('invNo')+ "&rid="+ id, 'divInvDtls');
                },
			...
			new Ajax.Request(module.ajaxUrl ,{
                ...
                getInvoices: function(){
                    module.execute('getInvoices', 'regNo='+ $F('regNo'), 'divInvoices');
                },
                printInvoice: function(invoiceNo){
                    var printWindow = window.open('./print?invoiceNo='+invoiceNo,'name','height=450,width=800,toolbar=no,menubar=no,directories=no,location=no,scrollbars=yes,status=no,resizable=no,fullscreen=no,top=200,left=200');
                    printWindow.focus();
                },
                delInvItem: function(id, itemName){
                    if(confirm("Delete '"+itemName+"'?")){
                        new Ajax.Request(module.ajaxUrl ,{
                            method:'post',
                            parameters: 'function=delInvItem&rid='+id,
                            requestHeaders: { Accept: 'application/json'},
                            onSuccess: function(request) {
                                response = request.responseText.evalJSON();
                                if(typeof response.success==='number' && response.success===1){
                                    g.info(response.message, { header : ' ' ,life: 5, speedout: 2  });
                                    invoice.editInvoice($F('invNo'));
                                }else{
                                    if(typeof response.message !== 'undefined'){
                                        g.error(response.message, { header : ' ' ,life: 5, speedout: 2 });
                                    }else{
