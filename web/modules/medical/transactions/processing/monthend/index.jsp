            ...
            var leave = {
                process: function(required){
                    if(module.validate(required)){
                        var yearFrom    = $F('yearFrom');
                        var yearTo      = $F('yearTo');
                        var monthFrom   = $F('monthFrom');
                        var monthTo     = $F('monthTo');
                        
                        if(yearFrom > yearTo){
                            g.error("Invalid selected periods", { header : ' ' ,life: 5, speedout: 2 });
                            return;
                        }
                        
                        if(yearFrom === yearTo){
                            var monthFrom = parseInt(monthFrom);
                            var monthTo = parseInt(monthTo);
