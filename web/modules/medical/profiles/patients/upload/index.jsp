...
//        out.println(filePath + fileName);
        ...
        query = "DELETE FROM HMPTPHOTOS WHERE PTNO = '"+ptNo+"'";
        Integer purged = stmt.executeUpdate(query);
        
        PreparedStatement ps;
        
        Integer id = system.generateId("HMPTPHOTOS", "ID");
        
        query = "INSERT INTO HMPTPHOTOS "
                    + "(ID, PTNO, PHOTO, FILENAME, FILETYPE, FILESIZE) "
                    + " VALUES (?, ?, ?, ?, ?, ?)";
...
<html>
    ...
    <body>
        <script type="text/javascript">
            <%
                if(errorCount > 0){%>
                    parent.patients.getUploadResponse(<%= errorCount%>, '<%= errorMsg%>');
                <%
                }else{%>
                    parent.patients.getUploadResponse();
                <%}
                %>
        </script>
    </body>
