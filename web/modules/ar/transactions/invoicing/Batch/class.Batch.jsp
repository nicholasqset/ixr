...
final class Batch{
    ...
    String table            = comCode+".ARINBATCHES";
...
//                    String batchTotalStr = sys.getOne("ARINDTLS", "SUM(AMOUNT)", "BATCHNO = '"+ batchNo+ "'");
...
//                    }
                    ...
                    html += "<td>"+ rtpUi+ "</td>";
                    html += "<td>"+ postedUi+ "</td>";
                    html += "<td>"+ sys.numberFormat(accountsReceivable.getInBatchAmount(batchNo).toString())+ "</td>";
                    html += "<td>"+ edit+ "</td>";
                    html += "</tr>";
	...
	html += "<td>";
        String btnSave = "";
        if(this.id == null){
            btnSave = gui.formButton(request.getContextPath(), "button", "btnSave", "Save", "save.png", "onclick = \"batch.save('desc');\"", "");
        }else{
            if(rtp == 0 || rtp == null){
                btnSave = gui.formButton(request.getContextPath(), "button", "btnSave", "Save", "save.png", "onclick = \"batch.save('desc batchSource');\"", "");
            }
        }
...
//            Integer batchPosted = accountsReceivable.postInBatch(this.batchNo, session, request);
            String batchPosted = accountsReceivable.postInBatch(this.batchNo, session, request);
