    ...
    <body>
        ...
        <script type="text/javascript">
            ...
            var invoices = {
                    ...
                    if(count <= 0){
                        ...
                        new Ajax.Autocompleter(
                                'batchNo', getResultTo, module.ajaxUrl,{
                                    ...
                                    minChars   : 2,
                                    frequency  : 1.0,
                                    afterUpdateElement : invoices.setBatch
                                }
                            );
                    }
                },
                setBatch: function(text, batch){
                    if(batch.id !== ''){
                        ...
                        invoices.getBatchProfile(batch.id);
                    }
                },
                ...
                searchCustomer: function(){
                    ...
                    if(count <= 0){
                        ...
                        new Ajax.Autocompleter(
                                ...
                                minChars   : 2,
                                frequency  : 1.0,
                                afterUpdateElement : invoices.setCustomer
                            });
                    }
                },
                setCustomer: function(text, customer){
                    if(customer.id !== ''){
                        ...
                        invoices.getCustomerProfile(customer.id);
                        /*
                         * invoices.getInEntries();
                         */
                    }
                },
                ...
                getEntryType: function(){
                    new Ajax.Request(module.ajaxUrl ,{
                        method:'post',
                        parameters: 'function=getEntryType&entryType='+ $F('entryType'),
                        requestHeaders: { Accept: 'application/json'},
                        onSuccess: function(request) {
                            response = request.responseText.evalJSON();
                            if(typeof response.success === 'number' && response.success === 1){
                                
                                if(typeof response.entryTypeLbl !== 'undefined' && $('tdEntryTypeLbl')) $('tdEntryTypeLbl').update(response.entryTypeLbl);
                                if(typeof response.entryTypeSel !== 'undefined' && $('tdEntryTypeSel')) $('tdEntryTypeSel').update(response.entryTypeSel);
                                if(typeof response.quantityUi !== 'undefined' && $('tdQuantity')) $('tdQuantity').update(response.quantityUi);
                                if(typeof response.amountUi !== 'undefined' && $('tdAmount')) $('tdAmount').update(response.amountUi);
                                
                            }else{
                                if(typeof response.message !== 'undefined'){
                                    g.error(response.message, { header : ' ' ,life: 5, speedout: 2 });
                                }else{
                                    g.error("Un-expected error occured while retrieving entry type.", { header : ' ' ,life: 5, speedout: 2 });
                                }
                            }
                        }
                    });
                ...
                getInEntries: function(){
                    module.execute('getInEntries', Form.serialize('frmModule'), 'dvInEntries');
                },
			...
			new Ajax.Request(module.ajaxUrl ,{
                            ...
                            onSuccess: function(request) {
                                ...
                                if(typeof response.success==='number' && response.success===1){
                                    ...
                                    if(typeof response.invoiceNo !== 'undefined'){
                                        ...
                                        if($('invoiceNoHd')) $('invoiceNoHd').value = response.invoiceNo;
                                    }
                                    
                                    g.info(response.message, { header : ' ' ,life: 5, speedout: 2  });
                                    if($('dvInEntries')) invoices.getInEntries();
                                }else{
                                    if(typeof response.message !== 'undefined'){
                                        g.error(response.message, { header : ' ' ,life: 5, speedout: 2 });
                                    }else{
                 ...
                 editInvDtls: function(sid){
                    new Ajax.Request(module.ajaxUrl ,{
                            method:'post',
                            parameters: 'function=editInvDtls&sid='+ sid,
                            requestHeaders: { Accept: 'application/json'},
                            onSuccess: function(request) {
                                response = request.responseText.evalJSON();
                                if(typeof response.success === 'number' && response.success===1){
                                    ...
                                    if(typeof response.sidUi !== 'undefined' && $('divInEntrySid')) $('divInEntrySid').update(response.sidUi);
                ...
                print: function(required){
                    if(module.validate(required)){
                        ...
                        var printWindow = window.open(
                                './print?batchNo='+ batchNo+ '&invoiceNo='+ invoiceNo, '', 'height=450, width=634, toolbar=no, menubar=no, directories=no, location=no, scrollbars=yes, status=no, resizable=no, fullscreen=no, top=200, left=200');
                        printWindow.focus();
                    }
                }
            };
