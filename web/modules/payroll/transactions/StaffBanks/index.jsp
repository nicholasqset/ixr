            ...
            var staffBanks = {
                    ...
                    if(count <= 0){
                        ...
                        new Ajax.Autocompleter(
                                'staff', getResultTo, module.ajaxUrl, {
                                    ...
                                    minChars   : 2,
                                    frequency  : 1.0,
                                    afterUpdateElement : staffBanks.setStaff
                                }
                            );
                    }
                },
                setStaff: function(text, staff){
                    if(staff.id !== ''){
                        ...
                        staffBanks.getStaffDtls(staff.id);
                    }
                },
                ...
                searchBkBranch: function(){
                    var count = Ajax.activeRequestCount;
                    if(count <= 0){
                        var getResultTo = 'bankBranchDiv';
                        new Ajax.Autocompleter(
                                'bankBranch', getResultTo, module.ajaxUrl, {
                                    paramName  : 'bkBranchCode',
                                    parameters : 'function=searchBkBranch',
                                    minChars   : 2,
                                    frequency  : 1.0,
                                    afterUpdateElement : staffBanks.setBankBranch
                                }
                            );
                    }
                },
                setBankBranch: function(text, bankBranch){
                    if(bankBranch.id !== ''){
                        if($('bkBranchCode')) $('bkBranchCode').value= bankBranch.id;
                        staffBanks.getBkBranchDtls(bankBranch.id);
                    }
                },
                getBkBranchDtls: function(bankBranch){
                    new Ajax.Request(module.ajaxUrl, {
                        method:'post',
                        parameters: 'function=getBkBranchDtls&bankBranch='+ bankBranch,
                        requestHeaders: { Accept: 'application/json'},
                        onSuccess: function(request) {
                            response = request.responseText.evalJSON();
                            if(typeof response.success === 'number' && response.success === 1){
                                
                                if(typeof response.bkBrachName !== 'undefined' && $('spBkBranchName')) $('spBkBranchName').update(response.bkBrachName);
