...
final class StaffItems{
    ...
    String table        = session.getAttribute("comCode")+".PYSTAFFITEMS";
    String view         = session.getAttribute("comCode")+".VIEWPYACTVTRANS";
...
//                    + session.getAttribute("maxRecord");
            ...
            String orderBy = "PYEAR DESC, PMONTH DESC, PFNO, ITEMCODE ";
            String limitSql = "";
            ...
            try{
                ...
                while(rs.next()){
                    ...
                    String pYear_       = rs.getString("PYEAR");
                    String pMonth_      = rs.getString("PMONTH");
                    String amount_      = rs.getString("AMOUNT");
	...
	html += "<td>"+ gui.formAutoComplete("staff", 13, this.id != null? ""+ this.pfNo: "", "staffItems.searchStaff", "pfNo", this.id != null? ""+ this.pfNo: "")+ " <span id = \"spStaffName\">"+ fullName+ "</span></td>";
	html += "</tr>";
        
        html += "<tr>";
	html += "<td nowrap>"+ gui.formIcon(request.getContextPath(), "page-edit.png", "", "")+" "+gui.formLabel("item", "Item")+"</td>";
	html += "<td>"+ gui.formAutoComplete("item", 13, this.id != null? ""+ this.itemCode: "", "staffItems.searchItem", "itemCode", this.id != null? ""+ this.itemCode: "")+ " <span id = \"spItemName\">"+ itemName+ "</span></td>";
	html += "</tr>";
        
        html += "<tr>";
	html += "<td>"+ gui.formIcon(request.getContextPath(), "calendar.png", "", "")+ gui.formLabel("pYear", "Payroll Year")+ "</td>";
	html += "<td>"+ gui.formSelect("pYear", session.getAttribute("comCode")+".FNFISCALPRD", "PYEAR", "", "PYEAR DESC", this.id != null? "PYEAR = "+ this.pYear: "PYEAR = "+sys.getPeriodYear(session.getAttribute("comCode").toString()), this.id != null? ""+ this.pYear: ""+sys.getPeriodYear(session.getAttribute("comCode").toString()), "", false)+ "</td>";
	html += "</tr>";
...
//	html += "<td>"+ gui.formMonthSelect("pMonth", this.id != null? this.pMonth: sys.getPeriodMonth(), "", true)+ "</td>";
	html += "<td>"+ gui.formOneMonthSelect("pMonth", this.id != null? this.pMonth: sys.getPeriodMonth(session.getAttribute("comCode").toString()), "", true)+ "</td>";
	html += "</tr>";
        
        html += "<tr>";
	html += "<td nowrap>"+gui.formIcon(request.getContextPath(),"coins.png", "", "")+" "+gui.formLabel("amount", "Amount")+"</td>";
        html += "<td>"+ gui.formInput("text", "amount", 20, this.id != null? this.amount+ "": "", "onkeyup = \"\"", "")+ "</td>";
	html += "</tr>";
        
        html += "<tr>";
	html += "<td>&nbsp;</td>";
	html += "<td>";
	html += gui.formButton(request.getContextPath(), "button", "btnSave", "Save", "save.png", "onclick = \"staffItems.save('item staff pYear pMonth amount');\"", "");
        if(this.id != null){
            html += gui.formButton(request.getContextPath(), "button", "btnDelete", "Delete", "delete.png", "onclick = \"staffItems.purge("+this.id+",'"+this.itemCode+"');\"", "");
        }
    ...
    public String searchItem(){
        ...
        html += gui.getAutoColsSearch(session.getAttribute("comCode")+".PYITEMS", "ITEMCODE, ITEMNAME", "", this.itemCode);
    ...
    public Object getItemDtls() throws Exception{
        ...
        if(this.itemCode == null || this.itemCode.trim().equals("")){
            ...
            String itemName = sys.getOne(session.getAttribute("comCode")+".PYITEMS", "ITEMNAME", "ITEMCODE = '"+ this.itemCode+ "'");
        ...
        try{
            ...
            if(this.id == null){
                ...
                query = "INSERT INTO "+ this.table+ " "
                        + "(ID, ITEMCODE, PFNO, PYEAR, PMONTH, AMOUNT, "
