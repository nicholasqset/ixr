    ...
    <body>
        ...
        <script type="text/javascript">
            ...
            var pca = {
                searchStaff: function(){
                    ...
                    if(count <= 0){
                        ...
                        new Ajax.Autocompleter(
                                ...
                                minChars   : 2,
                                frequency  : 1.0,
                                afterUpdateElement : pca.setStaff
                            });
                    }
                },
                ...
                getPcaEntries: function(){
                    module.execute('getPcaEntries', Form.serialize('frmModule'), 'dvPCA');
                },
                searchItem: function(){
                    var count = Ajax.activeRequestCount;
                    if(count <= 0){
                        var getResultTo = 'itemDiv';
                        new Ajax.Autocompleter(
                                'item', getResultTo, module.ajaxUrl, {
                                    paramName  : 'itemCode',
                                    parameters : 'function=searchItem',
                                    minChars   : 2,
                                    frequency  : 1.0,
                                    afterUpdateElement : pca.setItem
                                }
                            );
                    }
                },
                setItem: function(text, item){
                    if(item.id !== ''){
                        if($('itemCode')) $('itemCode').value= item.id;
                        pca.getItemDtls(item.id);
                    }
                },
                getItemDtls: function(item){
                    new Ajax.Request(module.ajaxUrl, {
                        method:'post',
                        parameters: 'function=getItemDtls&item='+ item,
                        requestHeaders: { Accept: 'application/json'},
                        onSuccess: function(request) {
                            response = request.responseText.evalJSON();
                            if(typeof response.success === 'number' && response.success === 1){
                ...
                getOldAmount: function(){
                    new Ajax.Request(module.ajaxUrl, {
                        method:'post',
                        parameters: 'function=getOldAmount&'+ Form.serialize('frmModule'),
                        requestHeaders: { Accept: 'application/json'},
                        onSuccess: function(request) {
                            response = request.responseText.evalJSON();
                             if(typeof response.oldAmount !== 'undefined'){
                                if($('oldAmount')) $('oldAmount').value = response.oldAmount;
                            }else{
                                g.error("Un-expected error occured while retrieving old newAmount.", { header : ' ' ,life: 5, speedout: 2 });
                            }
                        }
                    });
                },
                save: function(required){
                    var oldAmount = $('oldAmount')? $F('oldAmount'): '0';
                    var data = Form.serialize('frmModule')+ '&oldAmount='+ oldAmount;
			...
			new Ajax.Request(module.ajaxUrl ,{
                 ...
                 editPcaDtls: function(sid){
                    new Ajax.Request(module.ajaxUrl ,{
                            method:'post',
                            parameters: 'function=editPcaDtls&sid='+ sid,
                            requestHeaders: { Accept: 'application/json'},
                            onSuccess: function(request) {
                                response = request.responseText.evalJSON();
                                if(typeof response.success === 'number' && response.success === 1){
                                    ...
                                    if(typeof response.sidUi !== 'undefined' && $('dvPcaSid')) $('dvPcaSid').update(response.sidUi);
                                    
                                    if(typeof response.staffNo !== 'undefined' && $('staffNo')) $('staffNo').value = response.staffNo;
                                    if(typeof response.fullName !== 'undefined' && $('tdFullName')) $('tdFullName').update(response.fullName);
                                    if(typeof response.item !== 'undefined' && $('item')) $('item').value = response.item;
                                    if(typeof response.itemName !== 'undefined' && $('tdItemName')) $('tdItemName').update(response.itemName);
                                    if(typeof response.newAmount !== 'undefined' && $('newAmount')) $('newAmount').value = response.newAmount;
                                    if(typeof response.oldAmount !== 'undefined' && $('oldAmount')) $('oldAmount').value = response.oldAmount;
                                    if(typeof response.lineDesc !== 'undefined' && $('lineDesc')) $('lineDesc').value = response.lineDesc;
