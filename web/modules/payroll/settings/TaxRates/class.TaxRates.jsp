...
final class TaxRates{
    ...
    String table        = ""+session.getAttribute("comCode")+".PYTAXRATES";
    ...
    Double amttxd       = request.getParameter("amtTxd") != null && ! request.getParameter("amtTxd").trim().equals("")? Double.parseDouble(request.getParameter("amtTxd")): null;
...
//                    + session.getAttribute("maxRecord");
        ...
        if(this.id != null){
            try{
                ...
                while(rs.next()){
                    ...
                    this.amttxd     = rs.getDouble("AMTTXD");	
                }
            }catch (SQLException e){
                html += e.getMessage();
            }
	...
	html += "</tr>";
        ...
        html += "<tr>";
	html += "<td nowrap>"+gui.formIcon(request.getContextPath(),"coins.png", "", "")+" "+gui.formLabel("rate", "Tax Rate")+"</td>";
        html += "<td>"+ gui.formInput("text", "rate", 20, this.id != null? this.rate+ "": "", "onkeyup = \"\"", "")+ "</td>";
	html += "</tr>";
        
        html += "<tr>";
	html += "<td nowrap>"+gui.formIcon(request.getContextPath(),"money.png", "", "")+" "+gui.formLabel("amtTxd", "Taxed Amount")+"</td>";
        html += "<td>"+ gui.formInput("text", "amtTxd", 20, this.id != null? this.amttxd+ "": "", "onkeyup = \"\"", "")+ "</td>";
	html += "</tr>";
        
        html += "<tr>";
	html += "<td>&nbsp;</td>";
	html += "<td>";
	html += gui.formButton(request.getContextPath(), "button", "btnSave", "Save", "save.png", "onclick = \"taxRates.save('pYear amtMin amtMax rate amtTxd');\"", "");
        if(this.id != null){
            html += gui.formButton(request.getContextPath(), "button", "btnDelete", "Delete", "delete.png", "onclick = \"taxRates.purge("+this.id+",'"+this.amttxd+"');\"", "");
        }
