...
final class Batch{
    ...
    String table            = comCode+".GLBATCHES";
        
    Integer id          = (request.getParameter("id") != null && ! request.getParameter("id").trim().equals(""))? Integer.parseInt(request.getParameter("id")): null;
    ...
    String batchSrc     = request.getParameter("batchSource");
        ...
        if(recordCount > 0){
            ...
            try{
                ...
                while(rs.next()){
                    ...
                    String batchSrc     = rs.getString("BATCHSRC");
...
//                            postedUi = "Open";
                    ...
                    Double batchDrAmount = generalLedger.getBatchDrAmount(batchNo);
                    Double batchCrAmount = generalLedger.getBatchCrAmount(batchNo);
                    ...
                    html += "<td>"+ rtpUi+ "</td>";
                    html += "<td>"+ postedUi+ "</td>";
                    html += "<td>"+ sys.numberFormat(batchDrAmount.toString())+ "</td>";
                    html += "<td>"+ sys.numberFormat(batchCrAmount.toString())+ "</td>";
                    html += "<td>"+ edit+ "</td>";
                    html += "</tr>";
    ...
    public String getModule(){
        ...
        HashMap<String, String> batchSources = new HashMap();
        if(this.id != null){
            batchSources.put("GL-JE", "GL Journal Entry");
            batchSources.put("AP-IN", "AP Invoice");
            batchSources.put("AP-PY", "AP Payment");
            batchSources.put("AR-IN", "AR Invoice");
            batchSources.put("AR-PY", "AR Payment");
            batchSources.put("PO-PY", "PO Receipt");
            batchSources.put("IC-IU", "IC Internal Usage");
            batchSources.put("IC-PY", "IC Items Received");
            batchSources.put("PS-PY", "POS Receipt");
            batchSources.put("AM-AQ", "AM Acquisition");
            batchSources.put("AM-DP", "AM Depreciation");
            batchSources.put("AM-DI", "AM Disposal");
            batchSources.put("CB-CB", "CB Cash Book");
        }else{
            batchSources.put("GL-JE", "GL Journal Entry");
        }
	...
	html += "</tr>";
        ...
        html += "<tr>";
	html += "<td>"+ gui.formIcon(request.getContextPath(), "page-white-edit.png", "", "")+ gui.formLabel("batchSource", " Batch Source")+"</td>";
	html += "<td>"+ gui.formArraySelect("batchSource", 200, batchSources, this.id != null? this.batchSrc: "", true, "", true)+ "</td>";
	html += "</tr>";
...
//        if(this.id != null){
//            html += gui.formButton(request.getContextPath(), "button", "btnDelete", "Delete", "delete.png", "onclick = \"batches.purge("+this.id+",'"+this.batchDesc+"');\"", "");
//        }
    ...
    public Object rtp() throws Exception{
        ...
        try{
            ...
            Double batchDrAmount = generalLedger.getBatchDrAmount(this.batchNo);
            Double batchCrAmount = generalLedger.getBatchCrAmount(this.batchNo);
            
            if(batchDrAmount == 0){
                rts = 0;
                msg = "Invalid batch debit amount";
            }
            
            if(batchCrAmount == 0){
                rts = 0;
                msg = "Invalid batch credit amount";
            }
            
            if(! batchDrAmount.equals(batchCrAmount)){
                rts = 0;
                msg = "An unbalanced batch";
            }
    ...
    public Object post() throws Exception{
         ...
         try{
            ...
            Integer batchPosted = generalLedger.postBatch(this.batchNo, session, request);
