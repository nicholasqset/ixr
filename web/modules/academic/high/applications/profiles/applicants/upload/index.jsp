...
//        out.println(filePath + fileName);
        ...
        query = "DELETE FROM HMSTAFFPHOTOS WHERE STAFFNO = '"+staffNo+"'";
        Integer purged = stmt.executeUpdate(query);
        
        PreparedStatement ps;
        
        Integer id = system.generateId("HMSTAFFPHOTOS", "ID");
        
        query = "INSERT INTO HMSTAFFPHOTOS "
                    + "(ID, STAFFNO, PHOTO, FILENAME, FILETYPE, FILESIZE) "
                    + " VALUES (?, ?, ?, ?, ?, ?)";
...
<html>
    ...
    <body>
        <script type="text/javascript">
            <%
                if(errorCount > 0){%>
                    parent.staffs.getUploadResponse(<%= errorCount%>, '<%= errorMsg%>');
                <%
                }else{%>
                    parent.staffs.getUploadResponse();
                <%}
                %>
        </script>
    </body>
