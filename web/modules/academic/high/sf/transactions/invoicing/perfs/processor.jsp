    ...
    public Integer createInvDtls(String studentNo){
        ...
        if(this.createInvHdr(studentNo) == 1){
            ...
            if(invNo != null && ! invNo.trim().equals("")){
                if(sys.recordExists(""+this.comCode+".VIEWHGFSDETAILS", "ACADEMICYEAR = "+ this.academicYear+ " AND TERMCODE = '"+ this.termCode+ "' AND FORMCODE = '"+ this.formCode+ "' AND STUDTYPECODE = '"+ this.studTypeCode+ "' ")){
                    try{
                        Connection conn = ConnectionProvider.getConnection();
                        Statement stmt = conn.createStatement();

                        String query = "SELECT * FROM "+this.comCode+".VIEWHGFSDETAILS WHERE ACADEMICYEAR = "+ this.academicYear+ " AND TERMCODE     = '"+ this.termCode+ "' AND FORMCODE     = '"+ this.formCode+ "' AND STUDTYPECODE = '"+ this.studTypeCode+ "' ";
