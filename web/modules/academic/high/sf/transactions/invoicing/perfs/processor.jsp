...
//                    out.flush(); // used to send the echoed data to the client
    ...
    public Integer createInvHdr(String studentNo){
        ...
        if(system.recordExists("HGINVSHDR", "ACADEMICYEAR = "+ this.academicYear+" AND TERMCODE = '"+ this.termCode+ "' AND STUDENTNO = '"+ studentNo+ "' AND INVTYPE = '"+ invType+ "'")){
            invHdrCreated = 1;
        }else{
            try{
                ...
                Integer id      = system.generateId("HGINVSHDR", "ID");
                String invNo    = system.getNextNo("HGINVSHDR", "ID", "", "IN", 7);

                String query = "INSERT INTO HGINVSHDR "
    ...
    public Integer createInvDtls(String studentNo){
        ...
        if(this.createInvHdr(studentNo) == 1){
            String invType = "AT";
            String invNo = system.getOne("HGINVSHDR", "INVNO", "ACADEMICYEAR = "+ this.academicYear+" AND TERMCODE = '"+ this.termCode+ "' AND STUDENTNO = '"+ studentNo+ "' AND INVTYPE = '"+ invType+ "'");
            if(invNo != null && ! invNo.trim().equals("")){
                if(system.recordExists("VIEWHGFSDETAILS", "ACADEMICYEAR = "+ this.academicYear+ " AND TERMCODE = '"+ this.termCode+ "' AND FORMCODE = '"+ this.formCode+ "' AND STUDTYPECODE = '"+ this.studTypeCode+ "' ")){
                    try{
                        Connection conn = ConnectionProvider.getConnection();
                        Statement stmt = conn.createStatement();

                        String query = "SELECT * FROM VIEWHGFSDETAILS WHERE ACADEMICYEAR = "+ this.academicYear+ " AND TERMCODE     = '"+ this.termCode+ "' AND FORMCODE     = '"+ this.formCode+ "' AND STUDTYPECODE = '"+ this.studTypeCode+ "' ";
    ...
    public Integer insertInvDtls(String invNo, String itemCode, Double amount){
        ...
        Sys sys = new Sys();
        HttpSession session = request.getSession();
        if(system.recordExists("HGINVSDTLS", "INVNO = '"+ invNo+ "' AND ITEMCODE = '"+ itemCode+ "'")){
            invDtlsInserted = 1;
        }else{
            try{
                Connection conn = ConnectionProvider.getConnection();
                ...
                Integer id      = system.generateId("HGINVSDTLS", "ID");

                String query = "INSERT INTO HGINVSDTLS "
