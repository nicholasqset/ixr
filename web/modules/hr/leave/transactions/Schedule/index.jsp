            ...
            var schedule = {
                    ...
                    if(count <= 0){
                        ...
                        new Ajax.Autocompleter(
                                'staff', getResultTo, module.ajaxUrl, {
                                    ...
                                    minChars   : 2,
                                    frequency  : 1.0,
                                    afterUpdateElement : schedule.setStaff
                                }
                            );
                    }
                },
                setStaff: function(text, staff){
                    if(staff.id !== ''){
                        ...
                        schedule.getStaffDtls(staff.id);
                    }
                },
                ...
                getEntitlement: function(){
                    new Ajax.Request(module.ajaxUrl, {
                        method:'post',
                        parameters: 'function=getEntitlement&'+ Form.serialize('frmModule'),
                        requestHeaders: { Accept: 'application/json'},
                        onSuccess: function(request) {
                            response = request.responseText.evalJSON();
                            if(typeof response.success === 'number' && response.success === 1){
                                
                                if(typeof response.entitlement !== 'undefined' && $('entitlement')) $('entitlement').value = response.entitlement;
                ...
                getTotalBal: function(){
                    var balBf_          = $('balBf')? $F('balBf'): '0';
                    balBf_              = (balBf_.trim() !== '')? balBf_: 0;
                    var entitlement_    = $('entitlement')? $F('entitlement'): '0';
                    entitlement_        = (entitlement_.trim() !== '')? entitlement_: 0;
                    var taken_          = $('taken')? $F('taken'): '0';
                    taken_              = (taken_.trim() !== '')? taken_: 0;
                    
                    var balBf           = parseInt(balBf_);
                    var entitlement     = parseInt(entitlement_);
                    var taken           = parseInt(taken_);
                    
                    var balTotal        = balBf + entitlement - taken;
                    
                    $('balTotal').value = balTotal;
                },
