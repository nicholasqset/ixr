...
<html>
    ...
    <body>
        ...
        <script type="text/javascript">
            ...
            var leaveApr = {
                approve: function(id, docNo, desc){
                    if($('rtp_'+ id).checked === true){
                        if(confirm("Approve leave document '"+ docNo+ "'?")){
                            new Ajax.Request(module.ajaxUrl, {
                                method:'post',
                                parameters: 'function=approve&id='+ id+ "&docNo="+ docNo,
                ...
                post: function(id, docNo, desc){
                    if($('posted_'+ id).checked === true){
                        if(confirm("Post leaveApr '"+ desc+ "'?")){
                            new Ajax.Request(module.ajaxUrl, {
                                method:'post',
                                parameters: 'function=post&id='+ id+ "&docNo="+ docNo,
                                requestHeaders: { Accept: 'application/json'},
                                onSuccess: function(request) {
                                    response = request.responseText.evalJSON();
                                    if(typeof response.success==='number' && response.success===1){
