        ...
        <script type="text/javascript">
            ...
            var po = {
                searchSupplier: function(){
                    ...
                    if(count <= 0){
                        ...
                        new Ajax.Autocompleter(
                                ...
                                minChars   : 2,
                                frequency  : 1.0,
                                afterUpdateElement : po.setSupplier
                            });
                    }
                },
                setSupplier: function(text, customer){
                    if(customer.id !== ''){
                        ...
                        po.getSupplierProfile(customer.id);
                        /*
                         * po.getPoEntries();
                         */
                    }
                ...
                getPoEntries: function(){
                    module.execute('getPoEntries', Form.serialize('frmModule'), 'dvPoEntries');
                },
			...
			new Ajax.Request(module.ajaxUrl ,{
                            ...
                            onSuccess: function(request) {
                                ...
                                if(typeof response.success==='number' && response.success===1){
                                    ...
                                    if(typeof response.poNo !== 'undefined'){
                                        if($('poNo')) $('poNo').value = response.poNo;
                                        if($('poNoHd')) $('poNoHd').value = response.poNo;
                                    }
                                    
                                    g.info(response.message, { header : ' ' ,life: 5, speedout: 2  });
                                    if($('dvPoEntries')) po.getPoEntries();
                                }else{
                                    if(typeof response.message !== 'undefined'){
                                        g.error(response.message, { header : ' ' ,life: 5, speedout: 2 });
                                    }else{
                    ...
                    new Ajax.Request(module.ajaxUrl ,{
                            ...
                            onSuccess: function(request) {
                                ...
                                if(typeof response.success === 'number' && response.success===1){
                                    ...
                                    if(typeof response.sidUi !== 'undefined' && $('dvPoEntrySid')) $('dvPoEntrySid').update(response.sidUi);
                ...
                print: function(required){
                    if(module.validate(required)){
                        var poNo = $('poNo')? $F('poNo'): '';
                        var printWindow = window.open(
                                './print?poNo='+ poNo, '', 'height=450, width=800, toolbar=no, menubar=no, directories=no, location=no, scrollbars=yes, status=no, resizable=no, fullscreen=no, top=200, left=200');
                        printWindow.focus();
                    }
                }
            };
