...
//                    html += "<td>"+ fullName+ "</td>";
	...
	html += gui.formButton(request.getContextPath(), "button", "btnSave", "Save", "save.png", "onclick = \"requisition.save('entryDate requisitionDesc supplierNo pYear pMonth quantity item cost amount'); return false;\"", "");
	if(this.id != null){
            html += gui.formButton(request.getContextPath(), "button", "btnPrint", "Print", "printer.png", "onclick = \"requisition.print('requisitionNo'); return false;\"", "");
        }
	...
	html += "<td nowrap>"+ gui.formIcon(request.getContextPath(),"user-properties.png", "", "")+ gui.formLabel("supplierNo", " Supplier No.")+ "</td>";
        html += "<td>"+ gui.formAutoComplete("supplierNo", 13, this.id != null? this.supplierNo: "", "requisition.searchSupplier", "supplierNoHd", this.id != null? this.supplierNo: "")+ "</td>";
	...
	html += "<td colspan = \"3\">"+ gui.formSelect("item", comCode+".ICITEMS", "ITEMCODE", "ITEMNAME", "", "", "", "onchange = \"requisition.getItemAmount();\"", false)+ "</td>";
	html += "</tr>";
        
        html += "<tr>";
	html += "<td nowrap>"+ gui.formIcon(request.getContextPath(), "page-white-edit.png", "", "")+ gui.formLabel("quantity", " Quantity Ordered")+ "</td>";
	html += "<td id = \"tdQuantity\" colspan = \"3\">"+ gui.formInput("text", "quantity", 15, "", "onkeyup = \"requisition.getItemTotalAmount();\"", "")+ "</td>";
	html += "</tr>";
        
        html += "<tr>";
	html += "<td>"+ gui.formIcon(request.getContextPath(), "coins.png", "", "")+ gui.formLabel("cost", " Cost")+ "</td>";
	html += "<td id = \"tdCost\" colspan = \"3\" nowrap>"+ gui.formInput("text", "cost", 15, "", "onkeyup = \"requisition.getItemTotalAmount();\"", "")+ "</td>";
	html += "</tr>";
...
//        ICItem iCItem = new ICItem(this.itemCode);
        ...
        if(this.rqNo != null){
            try{
                ...
                if(this.sid == null){
                    ...
                    Integer sid = sys.generateId(comCode+".PORQDTLS", "ID");
                    
                    query = "INSERT INTO "+comCode+".PORQDTLS "
                            + "(ID, RQNO, ITEMCODE, "
                            + "QTY, UNITCOST, AMOUNT, "
                    ...
                    query = "UPDATE "+comCode+".PORQDTLS SET "
                ...
                if(saved == 1){
                    ...
                    obj.put("requisitionNo", this.rqNo);
                }else{
                    obj.put("success", new Integer(0));
    ...
    public String getRqNo(){
        ...
        if(this.rqNo == null){
            try{
                ...
                this.rqNo = sys.getNextNo(this.table, "ID", "", "RQ", 7);
                ...
                Integer rqHdrCreated = stmt.executeUpdate(query);
                
                if(rqHdrCreated == 1){
                    //has requisition no already
                }else{
                    this.rqNo = null;
                }
        ...
        if(sys.recordExists(comCode+".VIEWPORQDTLS", "RQNO = '"+ this.rqNo+ "'")){
            ...
            try{
                ...
                while(rs.next()){
                    ...
                    String editLink     = gui.formHref("onclick = \"requisition.editRqDtls("+ id +");\"", request.getContextPath(), "", "edit", "edit", "", "");
                    String removeLink   = gui.formHref("onclick = \"requisition.purge("+ id +", '"+ itemName +"');\"", request.getContextPath(), "", "delete", "delete", "", "");
    ...
    public Object purge(){
         ...
         try{
            ...
            if(this.id != null){
                String query = "DELETE FROM "+comCode+".PORQDTLS WHERE ID = "+this.id;
