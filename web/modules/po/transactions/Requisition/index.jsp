...
<html>
    <head>
        ...
        <title>Purchases Requisition</title>
        <% 
            ...
            var requisition = {
                searchSupplier: function(){
                    ...
                    if(count <= 0){
                        ...
                        new Ajax.Autocompleter(
                                ...
                                minChars   : 2,
                                frequency  : 1.0,
                                afterUpdateElement : requisition.setSupplier
                            });
                    }
                },
                setSupplier: function(text, customer){
                    if(customer.id !== ''){
                        ...
                        requisition.getSupplierProfile(customer.id);
                        /*
                         * requisition.getRqEntries();
                         */
                    }
                },
                    ...
                    new Ajax.Request(module.ajaxUrl ,{
                        ...
                        onSuccess: function(request) {
                            ...
                            if(typeof response.cost !== 'undefined'){
                                if($('cost')) $('cost').value = response.cost;
                                if($('amount')) $('amount').value = response.cost;
                            }
                        }
                    });
                },
                ...
                getRqEntries: function(){
                    module.execute('getRqEntries', Form.serialize('frmModule'), 'dvRqEntries');
                },
			...
			new Ajax.Request(module.ajaxUrl ,{
                            ...
                            onSuccess: function(request) {
                                ...
                                if(typeof response.success==='number' && response.success===1){
                                    ...
                                    if(typeof response.requisitionNo !== 'undefined'){
                                        if($('requisitionNo')) $('requisitionNo').value = response.requisitionNo;
                                        if($('requisitionNoHd')) $('requisitionNoHd').value = response.requisitionNo;
                                    }
                                    
                                    g.info(response.message, { header : ' ' ,life: 5, speedout: 2  });
                                    if($('dvRqEntries')) requisition.getRqEntries();
                                }else{
                                    if(typeof response.message !== 'undefined'){
                                        g.error(response.message, { header : ' ' ,life: 5, speedout: 2 });
                                    }else{
                 ...
                 editRqDtls: function(sid){
                    new Ajax.Request(module.ajaxUrl ,{
                            method:'post',
                            parameters: 'function=editRqDtls&sid='+ sid,
                            requestHeaders: { Accept: 'application/json'},
                            onSuccess: function(request) {
                                response = request.responseText.evalJSON();
                                if(typeof response.success === 'number' && response.success===1){
                                    ...
                                    if(typeof response.sidUi !== 'undefined' && $('dvRqEntrySid')) $('dvRqEntrySid').update(response.sidUi);
                ...
                print: function(required){
                    if(module.validate(required)){
                        var requisitionNo = $('requisitionNo')? $F('requisitionNo'): '';
                        var printWindow = window.open(
                                './print?requisitionNo='+ requisitionNo, '', 'height=450, width=800, toolbar=no, menubar=no, directories=no, location=no, scrollbars=yes, status=no, resizable=no, fullscreen=no, top=200, left=200');
                        printWindow.focus();
                    }
                }
            };
