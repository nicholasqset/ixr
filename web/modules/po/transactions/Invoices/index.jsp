            ...
            var invoice = {
                searchSupplier: function(){
                    ...
                    if(count <= 0){
                        ...
                        new Ajax.Autocompleter(
                                ...
                                minChars   : 2,
                                frequency  : 1.0,
                                afterUpdateElement : invoice.setSupplier
                            });
                    }
                },
                setSupplier: function(text, customer){
                    if(customer.id !== ''){
                        ...
                        invoice.getSupplierProfile(customer.id);
                        invoice.getPyNoUi();
                        /*
                         * invoice.getInEntries();
                         */
                    }
                },
                ...
                getPyNoUi: function(){
                    module.execute('getPyNoUi', Form.serialize('frmModule'), 'tdPoNo');
                },
                loadPyItems: function(required){
                    if(module.validate(required)){
                        new Ajax.Request(module.ajaxUrl, {
                            method:'post',
                            parameters: 'function=loadPyItems&'+ Form.serialize('frmModule'),
                            requestHeaders: { Accept: 'application/json'},
                            onSuccess: function(request) {
                                response = request.responseText.evalJSON();
                                if(typeof response.success==='number' && response.success===1){
                ...
                getPyItems: function(){
                    module.execute('getPyItems', Form.serialize('frmModule'), 'tdItems');
                },
                getItemPyDtls: function(){
                    new Ajax.Request(module.ajaxUrl ,{
                        method:'post',
                        parameters: 'function=getItemPyDtls&'+ Form.serialize('frmModule'),
                        requestHeaders: { Accept: 'application/json'},
                        onSuccess: function(request) {
                            response = request.responseText.evalJSON();
			...
			new Ajax.Request(module.ajaxUrl ,{
                            ...
                            onSuccess: function(request) {
                                ...
                                if(typeof response.success==='number' && response.success===1){
                                    ...
                                    /*
                                    if(typeof response.inNo !== 'undefined'){
                                        if($('inNo')) $('inNo').value = response.inNo;
                                    }
                                    */
                                    
                                    g.info(response.message, { header : ' ' ,life: 5, speedout: 2  });
                                    if($('dvInEntries')) invoice.getInEntries();
                                }else{
                                    if(typeof response.message !== 'undefined'){
                                        g.error(response.message, { header : ' ' ,life: 5, speedout: 2 });
                                    }else{
                 ...
                 editInDtls: function(sid){
                    new Ajax.Request(module.ajaxUrl ,{
                            method:'post',
                            parameters: 'function=editInDtls&sid='+ sid,
                            requestHeaders: { Accept: 'application/json'},
                            onSuccess: function(request) {
                                response = request.responseText.evalJSON();
                                if(typeof response.success === 'number' && response.success===1){
                                    ...
                                    if(typeof response.sidUi !== 'undefined' && $('dvInEntrySid')) $('dvInEntrySid').update(response.sidUi);
                ...
                post: function(id, inNo, desc){
                    if($('posted_'+ id)){
                        if($('posted_'+ id).checked === true){
                            if(confirm("Post Invoice '"+ desc+ "'?")){
                                new Ajax.Request(module.ajaxUrl, {
                                    method:'post',
                                    parameters: 'function=post&id='+ id+ "&inNo="+ inNo,
                                    requestHeaders: { Accept: 'application/json'},
                                    onSuccess: function(request) {
                                        response = request.responseText.evalJSON();
                                        if(typeof response.success==='number' && response.success===1){
