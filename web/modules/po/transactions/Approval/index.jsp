            ...
            var requisitions = {
                searchSupplier: function(){
                    ...
                    if(count <= 0){
                        ...
                        new Ajax.Autocompleter(
                                ...
                                minChars   : 2,
                                frequency  : 1.0,
                                afterUpdateElement : requisitions.setSupplier
                            });
                    }
                },
                setSupplier: function(text, customer){
                    if(customer.id !== ''){
                        ...
                        requisitions.getSupplierProfile(customer.id);
                        /*
                         * requisitions.getRqEntries();
                         */
                    }
                },
                ...
                approve: function(id, desc){
                    if($('approve_'+ id)){
                        if($('approve_'+ id).checked === true){
                            if(confirm("Approve requisition '"+ desc+ "' ?")){
                                new Ajax.Request(module.ajaxUrl, {
                                    method:'post',
                                    parameters: 'function=approve&id='+ id,
                                    requestHeaders: { Accept: 'application/json'},
                                    onSuccess: function(request) {
                                        response = request.responseText.evalJSON();
                                        if(typeof response.success==='number' && response.success===1){
                ...
                post: function(id, requisitionNo, desc){
                    if($('posted_'+ id)){
                        if($('posted_'+ id).checked === true){
                            if(confirm("Post Requisition '"+ desc+ "'?")){
                                new Ajax.Request(module.ajaxUrl, {
                                    method:'post',
                                    parameters: 'function=post&id='+ id+ "&requisitionNoHd="+ requisitionNo,
                                    requestHeaders: { Accept: 'application/json'},
                                    onSuccess: function(request) {
                                        response = request.responseText.evalJSON();
                                        if(typeof response.success==='number' && response.success===1){
