...
//            }
    ...
    <body>
        ...
        <script type="text/javascript">
            ...
            var bank = {
                toggleCodeUi: function(){
                    if($('autoCode').checked === true){
...
//                                    if(typeof response.code !== 'undefined' && response.code !== '' && $('autoCode').checked === true){
                                    if(typeof response.code !== 'undefined' && response.code !== ''){
                                        if($('autoCode')){
                                            if(('autoCode').checked === true){
                                                if($('code')) $('code').value = response.code;
                                            }
                                        }
                ...
                getUploadUI: function(id){
                    module.execute('getUploadUI', 'id='+ id, 'dvChqTps');
                },
                getChqList: function(id){
                    module.execute('getChqList', 'id='+ id, 'dvChqTps');
                },
                upload: function(required){
                    if(module.validate(required)){
                        var bank = $('code')? $F('code'): '';
                        var frmCheque = $('frmCheque');
                        var data = frmCheque.serialize();
                        frmCheque.action = "./upload/?"+ data+ '&bank='+ bank;
                        frmCheque.submit();
                        frmCheque.disable();
                    }
                },
                getUploadResponse: function(errorCount, errorMsg, sid){
                    if(typeof errorCount !== 'undefined' && errorCount > 0){
                        g.error(errorMsg, { header : ' ' , life: 5, speedout: 2 });
                    }else{
                        g.info("Template successfully uploaded", { header : ' ' ,life: 5, speedout: 2  });
                    }
                    bank.getChqList(sid);
                },
                removeTemplate: function(id, mid, template){
                   if(confirm("Delete '"+ template+ "'?")) {
                        var bank = $('code')? $F('code'): ''; 
                        new Ajax.Request(module.ajaxUrl, {
                            method:'post',
                            parameters: 'function=removeTemplate&sid='+ id+ '&code='+ bank+ '&template='+ template,
                            requestHeaders: { Accept: 'application/json'},
                            onSuccess: function(request) {
                                response = request.responseText.evalJSON();
                                if(typeof response.success==='number' && response.success===1){
                                    ...
                                    if(typeof response.message !== 'undefined'){
                                        ...
                                        g.error("Un-expected error occured while deleting record.", { header : ' ' ,life: 5, speedout: 2 });
                                    }
                                }
                            }
			});
                ...
                doDefault: function(id, mid, template){
                    if($('isDft_'+ id).checked === true){
                        if(confirm("Make template '"+ template+ "' default?")){
                            var bank = $('code')? $F('code'): ''; 
                            new Ajax.Request(module.ajaxUrl, {
                                method:'post',
                                parameters: 'function=doDefault&sid='+ id+ "&code="+ bank,
                                requestHeaders: { Accept: 'application/json'},
                                onSuccess: function(request) {
                                    response = request.responseText.evalJSON();
                                    if(typeof response.success==='number' && response.success===1){
                                        ...
                                        if(typeof response.message !== 'undefined'){
                                            ...
                                            g.error("Oops! An unexpected error occured.", { header : ' ' ,life: 5, speedout: 2 });
                                        }
                                    }
                                }
                            });
