...
//                    paymentUi = gui.formHref("onclick = \"javascript:alert('Already Paid.')\"", request.getContextPath(), "tick.png", "", "Already Paid", "", "");
        ...
        public Object purge() throws Exception {
            ...
            try {
                ...
                if (this.id != null) {
                    String cleared_ = sys.getOne(this.comCode + ".pspyhdr", "CLEARED", "PYNO IN (SELECT PYNO FROM " + this.comCode + ".PSPYDTLS WHERE ID = " + this.id + ")");
                    cleared_ = cleared_ != null ? cleared_ : "0";
                    Integer cleared = Integer.parseInt(cleared_);
                    String itemCode = sys.getOne(this.comCode + ".PSPYDTLS", "ITEMCODE", "ID = " + this.id);
                    String qty_ = sys.getOne(this.comCode + ".PSPYDTLS", "qty", "ID = " + this.id);
                    qty_ = qty_ != null ? qty_ : "0";
                    Double qty = Double.parseDouble(qty_);

                    if (cleared > 0) {
                        iC.effectItemQty(itemCode, qty, true, this.comCode);
                    }
