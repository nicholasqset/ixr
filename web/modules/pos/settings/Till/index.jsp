        ...
        <script type="text/javascript">
            ...
            var till = {
                searchUser: function(){
                    ...
                    if(count <= 0){
                        var getResultTo = 'userDiv';
                        new Ajax.Autocompleter(
                                'user', getResultTo, module.ajaxUrl, {
                                    paramName  : 'userId',
                                    parameters : 'function=searchUser',
                                    minChars   : 2,
                                    frequency  : 1.0,
                                    afterUpdateElement : till.setStaff
                                }
                            );
                    }
                },
                setStaff: function(text, user){
                    if(user.id !== ''){
                        if($('userId')) $('userId').value= user.id;
                        till.getUserDtls(user.id);
                    }
                },
                getUserDtls: function(user){
                    new Ajax.Request(module.ajaxUrl, {
                        method:'post',
                        parameters: 'function=getUserDtls&user='+ user,
                        requestHeaders: { Accept: 'application/json'},
                        onSuccess: function(request) {
                            response = request.responseText.evalJSON();
                            if(typeof response.success === 'number' && response.success === 1){
                                
                                if(typeof response.userName !== 'undefined' && $('spUserName')) $('spUserName').update(response.userName);
