            ...
            var sale = {
                searchCustomer: function(){
                    ...
                    if(count <= 0){
                        ...
                        new Ajax.Autocompleter(
                                ...
                                minChars   : 2,
                                frequency  : 1.0,
                                afterUpdateElement : sale.setCustomer
                            });
                    }
                },
                setCustomer: function(text, customer){
                    if(customer.id !== ''){
                        ...
                        sale.getCustomerProfile(customer.id);
                    }
                },
                ...
                searchItem: function(){
                    ...
                    if(count <= 0){
                        ...
                        new Ajax.Autocompleter(
                                ...
                                minChars   : 2,
                                frequency  : 1.0,
                                afterUpdateElement : sale.setItem
                            });
                    }
                },
			...
			new Ajax.Request(module.ajaxUrl ,{
                            ...
                            onSuccess: function(request) {
                                ...
                                if(typeof response.success==='number' && response.success===1){
                                    ...
                                    if(typeof response.saleNo !== 'undefined'){
                                        if($('saleNo')) $('saleNo').value = response.saleNo;
                                        if($('saleNoHd')) $('saleNoHd').value = response.saleNo;
                                    }
                                    
                                    g.info(response.message, { header : ' ' ,life: 5, speedout: 2  });
                                    if($('dvPyEntries')) sale.getPyEntries();
                                }else{
                                    if(typeof response.message !== 'undefined'){
                                        g.error(response.message, { header : ' ' ,life: 5, speedout: 2 });
                                    }else{
                ...
                getPaymentUi: function(saleNo){
                    var params = 'function=getPaymentUi&saleNoHd='+ saleNo;
                ...
                post: function(id, saleNo, desc){
                    if($('posted_'+ id)){
                        if($('posted_'+ id).checked === true){
                            if(confirm("Post Entry '"+ saleNo+ "'?")){
                                new Ajax.Request(module.ajaxUrl, {
                                    method:'post',
                                    parameters: 'function=post&id='+ id+ "&saleNoHd="+ saleNo,
                                    requestHeaders: { Accept: 'application/json'},
                                    onSuccess: function(request) {
                                        response = request.responseText.evalJSON();
                                        if(typeof response.success==='number' && response.success===1){
                ...
                print: function(required){
                    if(module.validate(required)){
                        var saleNo = $('saleNo')? $F('saleNo'): '';
                        var printWindow = window.open(
                                './print?saleNo='+ saleNo, '', 'height=464, width=525, toolbar=no, menubar=no, directories=no, location=no, scrollbars=yes, status=no, resizable=no, fullscreen=no, top=200, left=200');
                        printWindow.focus();
                    }
                }
            };
