...
//                        Integer posted          = rs.getInt("POSTED");
        ...
        public String getModule(){
            ...
            if(! cleared){
                html += gui.formButton(request.getContextPath(), "button", "btnSave", "Save", "save.png", "onclick = \"invoice.save('entryDate pyDesc pYear pMonth quantity itemNo price amount'); return false;\"", "");
            }
                
            html += gui.formButton(request.getContextPath(), "button", "btnPrint", "Print", "printer.png", "onclick = \"invoice.print('invoiceNo'); return false;\"", "");
            ...
            html += "initTabs(\'dhtmlgoodies_tabView1\', Array(\'Invoice/Order\'), 0, 625, 420, Array(false));";
            html += "</script>";
...
//            HashMap<String, String> tills = sys.getArray("SELECT TILLNO, TILLDESC FROM PSTILLS WHERE TILLNO IN (SELECT TILLNO FROM PSTILLS WHERE USERID = '"+sys.getLogUser(session)+"')");
            ...
            html += "<td>"+ gui.formAutoComplete("customerNo", 13, this.id != null? this.customerNo: "", "invoice.searchCustomer", "customerNoHd", this.id != null? this.customerNo: "")+ "</td>";
...
//            html += "<td id = \"tdItemName\">&nbsp;</td>";
            ...
            html += "<td id = \"tdCost\" colspan = \"3\" nowrap>"+ gui.formInput("text", "price", 15, "", "onkeyup = \"invoice.getItemTotalAmount();\"", "")+ "</td>";
            html += "</tr>";
...
//                obj.put("fullName", aRCustomerProfile.fullName);
        ...
        public Object save() throws Exception{
            ...
            if(qtyBal < 0){
                obj.put("success", new Integer(0));
                obj.put("message", "Stock not Enough on Item No '"+ iCItem.itemCode+"'.");
            }else{
                if(this.pyNo != null){
                    if(this.pyNo.equals("-1")){
                        ...
                        try{
                            ...
                            if(saved == 1){
                                ...
                                if(this.sid == null){
                                    obj.put("effectItemQty", this.effectItemQty(this.itemCode, this.qty, false));
                                }else{
                                    Double difQty   = this.qty - oldQty;
                                    
                                    obj.put("oldQty", oldQty);
                                    obj.put("difQty", difQty);
                                    ...
                                    if(difQty == 0){
                                        ...
                                        obj.put("effectItemQty", this.effectItemQty(this.itemCode, difQty, addEffect));
                                    }
                                }
                            }else{
                                obj.put("success", new Integer(0));
...
//                html += "<th style = \"text-align: right;\">Tax</th>";
                ...
                try{
                    ...
                    while(rs.next()){
                        ...
                        String editLink     = gui.formHref("onclick = \"invoice.editPoDtls("+ id +");\"", request.getContextPath(), "", "edit", "edit", "", "");
                        String removeLink   = gui.formHref("onclick = \"invoice.purge("+ id +", '"+ itemName +"', '"+ itemCode +"', "+ qty+ ");\"", request.getContextPath(), "", "delete", "delete", "", "");
...
//                html += "<td style = \"text-align: right; font-weight: bold;\">"+ sys.numberFormat(sumTax.toString()) +"</td>";
                ...
                html += "No invoice/order items record found.";
            }
...
//                    String query = "DELETE FROM RTPYDTLS WHERE ID IS NULL";
                    ...
                    if(purged == 1){
                        ...
                        obj.put("effectItemQty", this.effectItemQty(this.itemCode, this.qty, true));
                    }else{
                        obj.put("success", new Integer(0));
