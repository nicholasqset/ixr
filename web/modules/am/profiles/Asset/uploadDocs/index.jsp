...
//    }
    ...
    String assetNo         = request.getParameter("assetNoD");
...
//        query = "DELETE FROM AMASSETDOCS WHERE ASSETNO = '"+ assetNo+ "'";
//        Integer purged = stmt.executeUpdate(query);
        
        PreparedStatement ps;
        
        Integer id = system.generateId("AMASSETDOCS", "ID");
        
        query = "INSERT INTO AMASSETDOCS "
                    + "(ID, ASSETNO, DOC, FILENAME, FILETYPE, FILESIZE) "
                    + " VALUES (?, ?, ?, ?, ?, ?)";
        
                    
        ps = conn.prepareStatement(query);
        ps.setInt(1, id);
        ps.setString(2, assetNo);
...
<html>
    ...
    <body>
        <script type="text/javascript">
            <%
                if(errorCount > 0){%>
                    parent.asset.getDocUploadResp(<%= errorCount%>, '<%= errorMsg%>');
                <%
                }else{%>
                    parent.asset.getDocUploadResp();
                <%}
                %>
        </script>
    </body>
