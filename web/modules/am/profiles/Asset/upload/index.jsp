...
//        out.println(filePath + fileName);
        ...
        query = "DELETE FROM AMASSETPHOTOS WHERE ASSETNO = '"+assetNo+"'";
        Integer purged = stmt.executeUpdate(query);
        
        PreparedStatement ps;
        
        Integer id = system.generateId("AMASSETPHOTOS", "ID");
        
        query = "INSERT INTO AMASSETPHOTOS "
                    + "(ID, ASSETNO, PHOTO, FILENAME, FILETYPE, FILESIZE) "
                    + " VALUES (?, ?, ?, ?, ?, ?)";
...
<html>
    ...
    <body>
        <script type="text/javascript">
            <%
                if(errorCount > 0){%>
                    parent.asset.getUploadResponse(<%= errorCount%>, '<%= errorMsg%>');
                <%
                }else{%>
                    parent.asset.getUploadResponse();
                <%}
                %>
        </script>
    </body>
