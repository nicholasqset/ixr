...
<html>
    <head>
        ...
        <title>AM Acquisition Batches</title>
        <% 
    ...
    <body>
        ...
        <script type="text/javascript">
            ...
            var batch = {
                rtp: function(id, batchNo, desc){
                    if($('rtp_'+ id).checked === true){
                        if(confirm("Avail batch '"+ desc+ "' ready to post?")){
                            new Ajax.Request(module.ajaxUrl, {
                                method:'post',
                                parameters: 'function=rtp&id='+ id+ "&batchNo="+ batchNo,
                ...
                post: function(id, batchNo, desc){
                    if($('posted_'+ id).checked === true){
                        if(confirm("Post batch '"+ desc+ "'?")){
                            new Ajax.Request(module.ajaxUrl, {
                                method:'post',
                                parameters: 'function=post&id='+ id+ "&batchNo="+ batchNo,
                                requestHeaders: { Accept: 'application/json'},
                                onSuccess: function(request) {
                                    response = request.responseText.evalJSON();
                                    if(typeof response.success==='number' && response.success===1){
