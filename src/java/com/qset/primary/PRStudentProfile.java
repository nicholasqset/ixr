...
public class PRStudentProfile {
    ...
    public Double getFeesBalance(){
        ...
        String dr = system.getOneAgt("VIEWPROBS", "SUM", "AMOUNT", "SM", "STUDENTNO = '"+ this.studentNo+ "' AND (DOCTYPE = 'IN' OR DOCTYPE = 'DN')");
        if(dr == null){
            dr = "0";
        }
        ...
        String cr = system.getOneAgt("VIEWPROBS", "SUM", "AMOUNT", "SM", "STUDENTNO = '"+ this.studentNo+ "' AND (DOCTYPE = 'RC' OR DOCTYPE = 'CN')");
        if(cr == null){
            cr = "0";
        }
